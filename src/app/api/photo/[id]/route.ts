...
export async function GET(
  ...
  // нужно явно расширить список.
  //
  // ТЗ №16 B — BATCH добавлен ОСОЗНАННО. Складские страницы (карточка камня,
  // редактирование партии) показывают фото через этот же маршрут, и без него
  // картинка отдавала бы 404 сотруднику. Фактической разницы в доступе это не
  // создаёт: фото партии лежит в Vercel Blob с `access: "public"`, то есть его
  // URL открыт и так, а маршрут лишь прячет URL за cuid.
  //
  // ВАЖНО: на клиентскую QR-карточку фото партии НЕ попадает — там свой,
  // более узкий список `QR_PUBLIC_PHOTO_KINDS` (INTERIOR_AI / SAMPLE / SLAB),
  // и BATCH в него не входит. Клиент видит камень, а не паллету на складе.
